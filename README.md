...
public function save_settings() {
    ...
    $qry = $this->conn->prepare("SELECT * FROM system_settings WHERE id = 1");
    $qry->execute();
    $current = $qry->get_result()->fetch_assoc();
    $qry->close();
    ...
    $update = $this->conn->prepare("UPDATE system_settings SET name = ?, email = ?, contact = ?, about_content = ?, cover_img = ? WHERE id = 1");
    $update->bind_param("sssss", $name, $email, $contact, $about, $cover_img);
